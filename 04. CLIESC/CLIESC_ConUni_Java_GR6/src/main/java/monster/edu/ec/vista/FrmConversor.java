...
public class FrmConversor extends JFrame {
    ...
    private void initComponents() {
        ...
        lblDescripcion = new JLabel("Elige categoria, operacion y valor para convertir con el servicio REST.", SwingConstants.CENTER);
    ...
    private void aplicarCategoria(Categoria categoria) {
        if (categoria == null) categoria = Categoria.TEMPERATURA;
        categoriaActual = categoria;
        if (cbCategoria.getSelectedItem() != categoria) cbCategoria.setSelectedItem(categoria);
    ...
    private void actualizarDescripcionOperacion() {
        ...
        if (item != null) lblDescripcion.setText(item.descripcion);
    }
    ...
    private void btnCalcularActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            ...
            JOptionPane.showMessageDialog(this, "No se pudo consumir el servicio REST.\nError: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private Conversor ejecutarOperacion(String codigo, double valor) throws Exception {
        return switch (codigo) {
            case "CtoF"       -> ConversorControlador.convertirCtoF(valor);
            case "FtoC"       -> ConversorControlador.convertirFtoC(valor);
            case "CtoK"       -> ConversorControlador.convertirCtoK(valor);
            case "KtoC"       -> ConversorControlador.convertirKtoC(valor);
            case "FtoK"       -> ConversorControlador.convertirFtoK(valor);
            case "KtoF"       -> ConversorControlador.convertirKtoF(valor);
            case "KmAMetros"  -> ConversorControlador.convertirKmAMetros(valor);
            case "MetrosACm"  -> ConversorControlador.convertirMetrosACm(valor);
            case "PulgadasACm"-> ConversorControlador.convertirPulgadasACm(valor);
            case "PiesAMetros"-> ConversorControlador.convertirPiesAMetros(valor);
            case "MillasAKm"  -> ConversorControlador.convertirMillasAKm(valor);
            case "KgAGramos"  -> ConversorControlador.convertirKgAGramos(valor);
            case "GramosAMg"  -> ConversorControlador.convertirGramosAMg(valor);
            case "LibrasAKg"  -> ConversorControlador.convertirLibrasAKg(valor);
            case "OnzasAGramos"-> ConversorControlador.convertirOnzasAGramos(valor);
            case "ToneladasAKg"-> ConversorControlador.convertirToneladasAKg(valor);
            default -> null;
        };
    }
